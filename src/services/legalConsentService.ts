...
export class LegalConsentService {
  ...
  static async checkUserPendingPolicyUpdates(userId: string): Promise<LegalPolicyVersion[]> {
    ...
    try {
      ...
      // If user has no previous acceptances or is newly registered, do not interrupt their dashboard
      ...
      // Filter policies requiring re-acceptance only if user has an existing older version
