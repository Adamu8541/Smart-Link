...
export class APIProviderManager {
  ...
  static sanitizeConfig(provider: APIProviderConfig): APIProviderConfig {
    return {
      ...
      webhookSigningSecret: provider.webhookSigningSecret ? (provider.webhookSigningSecret.includes("••••") ? provider.webhookSigningSecret : `${provider.webhookSigningSecret.substring(0, 4)}••••••••`) : undefined,
