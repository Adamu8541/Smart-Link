...
export class AutomaticWalletFundingEngine {
   ...
   */
  static async processIncomingPaymentNotification(
    db: any,
    params: {
      payload: any;
  ...
  ): Promise<AutomaticFundingResult> {
    const res = await PaymentVerificationReconciliationEngine.verifyAndReconcilePayment(db, params);
