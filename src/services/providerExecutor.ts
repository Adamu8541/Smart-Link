  ...
  static async executeVirtualAccountCreation(
    ...
    if (endpoint) {
      ...
      try {
        ...
        if (fetchRes.ok && accountNumber) {
          return {
            success: true,
            providerName,
            providerCode,
            accountNumber,
            bankName: bankName || "Dedicated Bank",
            ...
            accounts: resBody?.accounts || (accountNumber ? [{ bankName: bankName || "Dedicated Bank", accountNumber }] : []),
          ...
          return {
            ...
            providerName,
            providerCode,
            error: resJson?.message || resJson?.error || `Provider ${providerName} did not return an account number (HTTP ${fetchRes.status})`,
            rawResponse: resJson,
          };
        }
      } catch (err: any) {
        ...
        return {
          ...
          providerName,
          providerCode,
          error: `Virtual account request to ${providerName} failed: ${err.message}`,
        };
      }
    }
    ...
    return {
      ...
      providerName,
      providerCode,
      error: `Virtual account creation failed: No active endpoint configured for provider ${providerName}.`,
    };
  }
