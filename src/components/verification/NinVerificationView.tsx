...
export const NinVerificationView: React.FC<NinVerificationViewProps> = ({
  ...
  // Search Method Mode
  const [searchMethod, setSearchMethod] = useState<NinSearchMethod>("BY_NIN");

  // Slip Format Selection
  const [selectedSlip, setSelectedSlip] = useState<SlipOptionConfig>(NIN_SLIP_OPTIONS[0]);

  // Form Inputs
  const [primaryInput, setPrimaryInput] = useState("");
  const [fullName, setFullName] = useState("");
  const [dateOfBirth, setDateOfBirth] = useState("");
  const [stateOfOrigin, setStateOfOrigin] = useState("Kano");
  const [customerPhone, setCustomerPhone] = useState("");
  const [verificationPurpose, setVerificationPurpose] = useState("KYC Identity Verification");
  ...
  const handleProceedToConfirmation = () => {
    ...
    if (searchMethod === "BY_NIN") {
      const validation = VerificationValidator.validateNIN(cleanInput);
      if (!validation.valid) {
    ...
    } else if (searchMethod === "BY_VNIN") {
      if (cleanInput.length < 16) {
        setInputError("Virtual NIN (vNIN) must be 16 alphanumeric characters.");
        return;
      }
    } else if (searchMethod === "BY_TRACKING_ID") {
      if (!cleanInput) {
        setInputError("Please enter the NIMC Tracking ID.");
        return;
      }
    } else if (searchMethod === "BY_DEMOGRAPHICS") {
      if (!fullName) {
    ...
    if (!userConsent) {
      setInputError("You must confirm user consent before querying NIMC identity records under NDPR regulations.");
      return;
    }

    // Balance check
    if (userBalance < selectedSlip.price) {
      setInputError(`Insufficient wallet balance (₦${userBalance.toLocaleString()}). You need ₦${selectedSlip.price.toLocaleString()} for this ${selectedSlip.name}. Please fund your wallet.`);
      return;
    }
  ...
  const handleConfirmAndExecute = async () => {
    ...
    try {
      const targetQuery = searchMethod === "BY_DEMOGRAPHICS" ? fullName : primaryInput.trim();
      ...
      if (stateOfOrigin) extraFields.stateOfOrigin = stateOfOrigin;
      ...
      if (res.success && res.result) {
        ...
        // Record compliance audit trail for NIMC/NDPR inquiry
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto p-3 sm:p-6 text-left">
      {/* Top Header Section with Real-Time Balance & Dynamic Service Fee */}
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-5 sm:p-6 shadow-sm space-y-4">
          ...
          <div className="flex items-center gap-3.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                Official federal identity search, instant verification, and multi-format slip generation
              </p>
            </div>
          </div>
        ...
        <div className="flex items-center gap-2 pt-2 border-t border-slate-100 dark:border-slate-800">
          ...
          >
            ...
            <span>NIN Verification &amp; Slip Generator</span>
          </button>
      ...
      {activeTab === "VERIFY" && (
        <div className="space-y-6">
          {stepMode === "INPUT" && (
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
              ...
              <div className="lg:col-span-7 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-5 sm:p-6 shadow-sm space-y-6">
                {/* 1. Slip Type Selector (Dropdown & Quick Card Selector) */}
                <div className="space-y-2.5">
                  <div className="flex items-center justify-between">
                    <label className="text-xs font-extrabold text-slate-900 dark:text-white uppercase tracking-wider flex items-center gap-2">
                      ...
                      <span>Select Output Slip Format</span>
                    </label>
                  ...
                  <div className="relative">
                    <select
                      ...
                      onChange={(e) => {
                        const opt = NIN_SLIP_OPTIONS.find((s) => s.id === e.target.value);
                        if (opt) setSelectedSlip(opt);
                      }}
                      className="w-full appearance-none px-4 py-3.5 bg-slate-50 dark:bg-slate-800/80 border border-slate-300 dark:border-slate-700 rounded-2xl text-xs font-bold text-slate-900 dark:text-white focus:outline-hidden focus:ring-2 focus:ring-emerald-500/20 pr-10 cursor-pointer shadow-xs"
                    >
                ...
                <div className="space-y-2 pt-3 border-t border-slate-100 dark:border-slate-800">
                  ...
                  <div className="flex flex-wrap gap-1.5 p-1 bg-slate-100 dark:bg-slate-800/60 rounded-2xl">
                    {[
                      { id: "BY_NIN", label: "11-Digit NIN", icon: Hash },
                      { id: "BY_PHONE", label: "Phone Number", icon: Phone },
                      { id: "BY_TRACKING_ID", label: "Tracking ID", icon: FileText },
                      { id: "BY_VNIN", label: "Virtual NIN", icon: Sparkles },
                      { id: "BY_DEMOGRAPHICS", label: "Demographics", icon: User },
                    ].map((m) => {
                      ...
                      return (
                        <button
                          ...
                          type="button"
                          onClick={() => {
                            setSearchMethod(m.id as NinSearchMethod);
                            setInputError(null);
                          }}
                          className={`flex-1 min-w-[100px] py-2 px-2.5 rounded-xl text-xs font-bold transition-all cursor-pointer flex items-center justify-center gap-1.5 ${
                            isCurr
                ...
                <div className="space-y-4">
                  {searchMethod === "BY_NIN" && (
                    <div className="space-y-1.5">
                      <div className="flex items-center justify-between">
                        <label htmlFor="ninPrimaryInput" className="block text-xs font-bold text-slate-800 dark:text-slate-200">
                      ...
                      <p className="text-[11px] text-slate-500 dark:text-slate-400 flex items-center gap-1">
                        ...
                        <span>Enter the 11 numeric digits printed on the NIMC slip or ID card.</span>
                      </p>
                    </div>
                  )}
                  ...
                  {searchMethod === "BY_PHONE" && (
                    <div className="space-y-1.5">
                      <div className="flex items-center justify-between">
                        <label htmlFor="ninPhoneInput" className="block text-xs font-bold text-slate-800 dark:text-slate-200">
                      ...
                      <input
                        ...
                        className="w-full px-4 py-3.5 text-base bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl focus:outline-hidden focus:ring-2 focus:ring-emerald-500/20 font-mono tracking-wider text-slate-900 dark:text-white"
                      />
                      <p className="text-[11px] text-slate-500 dark:text-slate-400">
                        Searches NIMC database for NIN records linked to this registered phone number.
                      </p>
                    </div>
                  )}

                  {searchMethod === "BY_TRACKING_ID" && (
                    <div className="space-y-1.5">
                      <label htmlFor="ninTrackingInput" className="block text-xs font-bold text-slate-800 dark:text-slate-200">
                        NIMC Enrolment Tracking ID <span className="text-rose-500">*</span>
                      </label>
                      <input
                        id="ninTrackingInput"
                        type="text"
                        value={primaryInput}
                        onChange={(e) => {
                          setPrimaryInput(e.target.value.toUpperCase());
                          if (inputError) setInputError(null);
                        }}
                        placeholder="e.g. TRK-892182049"
                        className="w-full px-4 py-3.5 text-base bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl focus:outline-hidden focus:ring-2 focus:ring-emerald-500/20 font-mono text-slate-900 dark:text-white uppercase"
                      />
                    </div>
                  )}

                  {searchMethod === "BY_VNIN" && (
                    <div className="space-y-1.5">
                      <div className="flex items-center justify-between">
                        <label htmlFor="ninVninInput" className="block text-xs font-bold text-slate-800 dark:text-slate-200">
                          16-Character Virtual NIN (vNIN) <span className="text-rose-500">*</span>
                        </label>
                        <span className="text-[11px] font-mono text-slate-400">
                          {primaryInput.length}/16 chars
                        </span>
                      </div>
                      <input
                        id="ninVninInput"
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div className="sm:col-span-2 space-y-1">
                        <label className="block text-xs font-bold text-slate-800 dark:text-slate-200">
                          Full Legal Name (First Name &amp; Surname) <span className="text-rose-500">*</span>
                        </label>
                        <input
                          type="text"
                          value={fullName}
                  ...
                  {/* Optional Customer Phone / Purpose */}
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
                    ...
                    <div className="space-y-1">
                      ...
                      >
                        ...
                        <option value="Account Opening">Bank Account Opening</option>
                        <option value="ID Card Issuance">Plastic ID Card Issuance</option>
                        <option value="SIM Registration">SIM Registration / Swap</option>
                        <option value="Employment Screening">Employment Screening</option>
                        <option value="Embassy / Visa Application">Embassy / Visa Application</option>
                      </select>
                    </div>
                  </div>
                  ...
                  <IdentityVerificationConsentNotice
                    serviceName="National Identity Management Commission (NIMC) NIN Verification"
              ...
              {/* Right Column: Live Interactive Slip Preview Card with Dynamic Price */}
          ...
          <ConfirmationDialog
            ...
            serviceName={`NIN Verification — ${selectedSlip.name}`}
          ...
          {stepMode === "LOADING" && (
            <VerificationLoader
              ...
              serviceTitle={`NIN Verification (${selectedSlip.name})`}
              providerName="NIMC Federal Identity Gateway"
            />
          )}
          ...
          {stepMode === "SUCCESS" && result && (
            <div className="space-y-6">
              ...
              <div className="p-4 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800 rounded-2xl flex flex-wrap items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500 dark:text-slate-400">
                      Generated with scannable 2D QR validation seal and federal watermarks
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-extrabold flex items-center gap-2 shadow-md shadow-emerald-600/20 cursor-pointer"
                  >
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-4 border-b border-slate-100 dark:border-slate-800">
            <div>
              <h3 className="text-sm font-bold text-slate-900 dark:text-white flex items-center gap-2">
                ...
                <span>NIN Verification &amp; Slip History</span>
              </h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Audit log of all NIN queries and slip generation records on your account
              </p>
            </div>
          ...
          {historyLoading ? (
            ...
            <div className="py-12 text-center space-y-2">
              ...
              <p className="text-xs font-semibold text-slate-600 dark:text-slate-400">
                No NIN Records Found
              </p>
              <p className="text-[11px] text-slate-400">
                Perform your first NIN query using the generator above.
              </p>
            </div>
          ) : (
                    ...
                    <button
                      ...
                      onClick={() => {
                        const stdRes: StandardizedVerificationResult = {
                          ...
                          message: "NIN Verification Record",
                          data: item.data || null,
                      ...
                      className="px-3 py-1.5 bg-emerald-50 hover:bg-emerald-100 dark:bg-emerald-950/60 dark:hover:bg-emerald-900/60 text-emerald-700 dark:text-emerald-300 rounded-xl text-xs font-bold flex items-center gap-1.5 border border-emerald-300 dark:border-emerald-800 cursor-pointer"
                    >
