...
import AdminModule2TestPanel from "../AdminModule2TestPanel";
...
export default function AdminDashboardHome({
  ...
  const metricCards = [
    ...
    { label: "API Gateway Status", value: "OPERATIONAL", sub: "Monnify, OPay, VTU", icon: Server, color: "text-emerald-400", border: "border-emerald-800/40" },
  ];
    ...
    <div className="space-y-6">
      ...
      >
        <div className="space-y-3 max-w-2xl">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-950 border border-blue-800 text-blue-400 font-semibold text-xs">
          ...
          <div>
            ...
            <p className="text-xs md:text-sm text-slate-400 mt-1 leading-relaxed">
              {roleDef.description} • Role: <strong className="text-white">{session.role}</strong>
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3 pt-1 text-xs text-slate-300">
            <div className="flex items-center gap-1.5 px-3 py-1 rounded-xl bg-slate-950 border border-slate-800">
              ...
              <span>Session Expiry: <strong className="text-white font-mono">30 Min Timeout</strong></span>
            </div>

            <div className="flex items-center gap-1.5 px-3 py-1 rounded-xl bg-emerald-950 border border-emerald-800 text-emerald-300">
              <Activity className="h-3.5 w-3.5 text-emerald-400 animate-pulse" />
              <span>Gateway Status: <strong className="text-emerald-200">100% Operational</strong></span>
            </div>
          </div>
        </div>
      ...
      <div className="bg-slate-900/60 border border-slate-800 rounded-3xl p-5 space-y-3">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="p-1.5 rounded-xl bg-slate-800 text-slate-400 hover:text-white transition-colors cursor-pointer text-xs flex items-center gap-1"
          >
          ...
          {loading
            ...
            : metricCards.map((card) => {
                ...
                return (
                  <div
                    key={card.label}
                    className={`bg-slate-900 border ${card.border} rounded-2xl p-5 space-y-2 hover:border-slate-700 transition-all group`}
                  >
                    ...
                    <p className="text-xl font-extrabold text-white truncate">{card.value}</p>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 space-y-4">
          ...
          <div className="space-y-3">
            <div className="p-4 bg-slate-950 border border-slate-800 rounded-2xl space-y-1">
              <div className="flex items-center justify-between">
                <p className="text-xs font-bold text-white">Scheduled Maintenance Window — Monnify & OPay Gateway</p>
                <span className="px-2 py-0.2 rounded bg-rose-950 border border-rose-800 text-rose-400 font-mono text-[9px] font-bold">HIGH</span>
              </div>
            ...
            <div className="p-4 bg-slate-950 border border-slate-800 rounded-2xl space-y-1">
              <div className="flex items-center justify-between">
                <p className="text-xs font-bold text-white">NIMC Identity Verification Rate Adjustment</p>
                <span className="px-2 py-0.2 rounded bg-amber-950 border border-amber-800 text-amber-400 font-mono text-[9px] font-bold">MEDIUM</span>
              </div>
              <p className="text-xs text-slate-400 leading-relaxed">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="text-blue-400 hover:underline text-xs font-semibold flex items-center gap-1 cursor-pointer"
            >
          ...
          <div className="space-y-2.5">
            {[
              ...
              { text: "Monnify Auto-Funding webhook processed ₦50,000 credit", time: "4 mins ago", type: "WEBHOOK" },
              ...
              <div key={idx} className="p-3 bg-slate-950 border border-slate-800/80 rounded-2xl flex items-center justify-between text-xs">
                <div className="flex items-center gap-2.5 min-w-0">
                  ...
                  <p className="text-slate-300 truncate">{feed.text}</p>
                </div>
