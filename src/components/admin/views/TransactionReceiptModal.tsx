  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-200">
      <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-xl w-full overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 border-b border-slate-800 flex items-center justify-between bg-slate-950/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 font-mono">Ref: {transaction.smartLinkRef || transaction.id}</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <div className="p-6 md:p-8 space-y-6 overflow-y-auto bg-slate-900 print:bg-white print:text-black">
          {/* SmartLink Header */}
          <div className="text-center pb-6 border-b border-slate-800/80 print:border-black">
            ...
            <p className="text-xs text-slate-400 print:text-gray-600">Smart Link Digital Identity & Payment Infrastructure</p>
            <p className="text-[10px] text-slate-500 font-mono mt-0.5 print:text-gray-500">Official Electronic Receipt • Issued {transaction.date} {transaction.time}</p>
          </div>
          ...
          <div className="flex justify-center">
            <div className={`px-4 py-1.5 rounded-full border text-xs font-bold uppercase tracking-wider flex items-center gap-2 ${getStatusColor(transaction.status)}`}>
              {transaction.status === "SUCCESSFUL" && <CheckCircle2 className="h-4 w-4 text-emerald-400" />}
              {transaction.status === "FAILED" && <AlertTriangle className="h-4 w-4 text-rose-400" />}
              {transaction.status === "PENDING" && <Clock className="h-4 w-4 text-amber-400 animate-spin" />}
              {transaction.status === "REFUNDED" && <RefreshCw className="h-4 w-4 text-blue-400" />}
              <span>{transaction.status}</span>
            </div>
          </div>

          {/* Reference Numbers Table */}
          <div className="bg-slate-950/60 border border-slate-800/80 rounded-2xl p-4 space-y-2 text-xs print:bg-gray-100 print:border-gray-300">
            <div className="flex justify-between items-center">
              <span className="text-slate-400 print:text-gray-600">SmartLink Ref:</span>
              <span className="font-mono font-bold text-white print:text-black">{transaction.smartLinkRef || transaction.id}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-slate-400 print:text-gray-600">Provider Ref:</span>
              <span className="font-mono font-bold text-slate-300 print:text-black">{transaction.providerRef || "PRV-PENDING"}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-slate-400 print:text-gray-600">Payment Gateway:</span>
              <span className="font-medium text-slate-300 print:text-black">{transaction.provider || "VTU Gateway"}</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 text-xs">
            <div className="p-3.5 bg-slate-950/40 border border-slate-800/60 rounded-xl space-y-1">
              ...
              <p className="text-slate-400 text-[11px] truncate">{user?.email || transaction.userEmail}</p>
              <p className="text-slate-400 text-[11px] font-mono">{user?.phoneNumber || transaction.userPhone}</p>
            </div>
            <div className="p-3.5 bg-slate-950/40 border border-slate-800/60 rounded-xl space-y-1">
              ...
              <p className="text-slate-400 text-[11px] truncate">{transaction.walletUsed || "Main Wallet Float"}</p>
              <p className="text-emerald-400 text-[10px] font-bold">100% Encrypted Ledger</p>
            </div>
          </div>

          {/* Line Items Breakdown */}
          <div className="border border-slate-800/80 rounded-2xl overflow-hidden text-xs">
            <div className="bg-slate-950/80 p-3 font-bold text-slate-300 border-b border-slate-800 flex justify-between">
            ...
            <div className="p-4 space-y-3 bg-slate-900">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-[11px] text-slate-400 mt-0.5">{transaction.description || "Digital Service Execution"}</p>
                </div>
              ...
              <div className="flex justify-between items-center font-bold text-sm text-emerald-400 pt-3 border-t border-slate-800">
          ...
          <div className="pt-2 text-center space-y-1">
            ...
            <p className="text-[10px] text-slate-500 font-mono">For support inquiries, contact support@smartlink.com or +234 803 123 4567.</p>
          </div>
        </div>
        ...
        <div className="p-5 border-t border-slate-800 bg-slate-950/80 flex items-center justify-between gap-3">
          ...
          >
            <Share2 className="h-4 w-4 text-blue-400" /> Share
          </button>
          <div className="flex items-center gap-3">
            <button
              type="button"
            ...
            <button
              ...
              className="py-2.5 px-5 bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold rounded-xl flex items-center gap-2 cursor-pointer shadow-lg shadow-blue-600/20 transition-all"
            >
