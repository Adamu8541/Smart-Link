        ...
        <div className="w-screen max-w-2xl bg-slate-900 border-l border-slate-800 shadow-2xl flex flex-col">
          ...
          <div className="p-6 border-b border-slate-800 bg-slate-950/60 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <span className="text-[10px] font-bold uppercase tracking-wider text-purple-400">Transaction Investigation</span>
                <h2 className="text-lg font-bold text-white">SmartLink Reference Audit</h2>
                <p className="text-xs font-mono text-slate-400">{tx?.smartLinkRef || transactionId}</p>
              </div>
            </div>
            <button
              type="button"
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            {loading ? (
              ...
              <div className="p-4 bg-rose-950/50 border border-rose-800/80 rounded-2xl text-xs text-rose-300 flex items-center gap-3">
              ...
              <>
                ...
                <div className="flex items-center justify-between p-4 bg-slate-950/80 border border-slate-800 rounded-2xl">
                  ...
                  <button
                    ...
                    className="py-1.5 px-3 bg-blue-950 hover:bg-blue-900 border border-blue-800/80 text-blue-300 text-xs font-bold rounded-xl flex items-center gap-1.5 cursor-pointer transition-colors"
                  >
                ...
                <div className="bg-slate-950/60 border border-slate-800 rounded-2xl p-5 space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-xs">
                    <div>
                      <span className="text-slate-400 block text-[10px] uppercase font-bold">SmartLink Reference</span>
                      <span className="font-mono font-bold text-white">{tx.smartLinkRef}</span>
                    </div>
                    <div>
                      <span className="text-slate-400 block text-[10px] uppercase font-bold">Provider Reference</span>
                      <span className="font-mono font-bold text-slate-300">{tx.providerRef || "N/A"}</span>
                    </div>
                    <div>
                      <span className="text-slate-400 block text-[10px] uppercase font-bold">Provider Gateway</span>
                      <span className="font-medium text-slate-200">{tx.providerName}</span>
                    </div>
                    <div>
                      <span className="text-slate-400 block text-[10px] uppercase font-bold">Service Requested</span>
                      <span className="font-bold text-emerald-400">{tx.serviceName || tx.serviceType}</span>
                    </div>
                    <div>
                    ...
                    <div>
                      <span className="text-slate-400 block text-[10px] uppercase font-bold">Charges / Fees</span>
                      <span className="font-mono text-slate-300">₦{(tx.charges || 0).toLocaleString("en-NG", { minimumFractionDigits: 2 })}</span>
                    </div>
                  </div>

                  {/* Verification Extra Result payload if available */}
                  {tx.verificationResult && (
                    <div className="mt-3 p-3 bg-slate-900 border border-slate-800 rounded-xl text-xs space-y-1">
                      <span className="text-[10px] font-bold text-cyan-400 uppercase tracking-wider">Identity Verification Result</span>
                ...
                <div className="bg-slate-950/60 border border-slate-800 rounded-2xl p-5 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-xs">
                    ...
                    <div>
                      <span className="text-slate-400 block text-[10px] uppercase font-bold">User Email</span>
                      <span className="text-slate-300">{user?.email || "N/A"}</span>
                    </div>
                    <div>
                      <span className="text-slate-400 block text-[10px] uppercase font-bold">Phone Number</span>
                      <span className="font-mono text-slate-300">{user?.phoneNumber || "N/A"}</span>
                    </div>
                    <div>
                      <span className="text-slate-400 block text-[10px] uppercase font-bold">User UID</span>
                      <span className="font-mono text-[11px] text-slate-400">{user?.userId || user?.uid}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-slate-950/60 border border-slate-800 rounded-2xl p-5 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-xs">
                    ...
                    <div>
                      <span className="text-slate-400 block text-[10px] uppercase font-bold">Wallet / Source</span>
                      <span className="text-slate-300">{tx.walletUsed}</span>
                    </div>
                    <div>
                      <span className="text-slate-400 block text-[10px] uppercase font-bold">Previous Balance</span>
                      <span className="font-mono text-slate-400">₦{(tx.previousBalance || 0).toLocaleString()}</span>
                    </div>
                    <div>
                      <span className="text-slate-400 block text-[10px] uppercase font-bold">New Balance</span>
                      <span className="font-mono text-emerald-400 font-bold">₦{(tx.newBalance || 0).toLocaleString()}</span>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="relative pl-6 space-y-4 before:absolute before:left-2.5 before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-800">
                    {timeline.map((step: any, idx: number) => (
                      <div key={idx} className="relative flex flex-col text-xs space-y-0.5">
                        <div className="absolute -left-6 top-0.5 h-3.5 w-3.5 rounded-full border-2 border-slate-900 bg-emerald-500"></div>
                ...
                <div className="bg-slate-950/60 border border-slate-800 rounded-2xl p-5 space-y-4">
                  ...
                  {noteSuccess && (
                    <div className="p-2.5 bg-emerald-950/80 border border-emerald-800 text-xs text-emerald-300 rounded-xl">
                      {noteSuccess}
                    </div>
                  )}
                  ...
                  {notes.length > 0 ? (
                    <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                      {notes.map((n: any) => (
                        <div key={n.id} className="p-3 bg-slate-900 border border-slate-800/80 rounded-xl text-xs space-y-1">
                          <div className="flex justify-between text-[10px] text-slate-400 font-mono">
                            <span className="font-bold text-slate-300">{n.adminEmail}</span>
                    ...
                    <p className="text-xs text-slate-500 italic">No internal admin notes attached yet.</p>
                  )}
                  ...
                  <form onSubmit={handleAddNote} className="space-y-2 pt-2 border-t border-slate-800">
                    <textarea
                      ...
                      className="w-full bg-slate-900 border border-slate-800 rounded-xl p-3 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-cyan-500 transition-colors"
                    />
                    <div className="flex justify-end">
                      <button
                        type="submit"
                        disabled={submittingNote || !newNote.trim()}
                        className="py-2 px-4 bg-cyan-600 hover:bg-cyan-500 disabled:opacity-50 text-white text-xs font-bold rounded-xl flex items-center gap-1.5 cursor-pointer transition-colors shadow-md"
                      >
                ...
                <div className="p-5 bg-slate-950/80 border border-slate-800 rounded-2xl space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    {tx.status !== "SUCCESSFUL" && tx.status !== "COMPLETED" ? (
                      <button
                        ...
                        className="py-2.5 px-4 bg-amber-950 hover:bg-amber-900 border border-amber-800/80 text-amber-300 text-xs font-bold rounded-xl flex items-center justify-center gap-2 cursor-pointer transition-colors"
                      >
                    ...
                    <button
                      ...
                      className="py-2.5 px-4 bg-rose-950 hover:bg-rose-900 border border-rose-800/80 text-rose-300 text-xs font-bold rounded-xl flex items-center justify-center gap-2 cursor-pointer transition-colors"
                    >
                  ...
                  {showRetryConfirm && (
                    <div className="p-4 bg-amber-950/80 border border-amber-800/80 rounded-xl text-xs space-y-3">
                      ...
                      <div className="flex justify-end gap-2">
                        <button
                          ...
                          className="py-1.5 px-3 bg-slate-800 text-slate-300 rounded-lg font-bold"
                        >
                          Cancel
                        </button>
                        <button
                          ...
                          disabled={isRetrying}
                          className="py-1.5 px-4 bg-amber-600 hover:bg-amber-500 text-white rounded-lg font-bold flex items-center gap-1.5"
                        >
