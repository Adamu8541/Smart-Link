...
export interface GatewayRoutingRule {
  ...
  tertiaryProviderName?: string;
  fallbackProviderId?: string;
  fallbackProviderName?: string;
  timeoutMs: number; // e.g. 6000
  maxRetries: number; // e.g. 2
  autoFailover: boolean;
  circuitBreakerThreshold: number; // consecutive errors before tripping (e.g. 3)
  circuitBreakerResetMs: number; // cooldown before testing again (e.g. 60000)
  enabled: boolean;
  updatedAt: string;
}
...
export interface GatewayHealthMetric {
  ...
  failoverTriggeredCount: number;
  consecutiveFailures: number;
  circuitBreakerTripped: boolean;
  lastPingAt?: string;
  lastPingStatus?: "SUCCESS" | "FAILED" | "TIMEOUT";
...
export interface GatewayFailoverLog {
  ...
  fallbackProviderId: string;
  fallbackProviderName: string;
  fallbackStatus: "SUCCESS" | "FAILED" | "PENDING";
  timestamp: string;
}

export interface BackgroundVerificationJob {
  id: string;
  reference: string;
  userId: string;
  userEmail?: string;
  ...
  providerName: string;
  providerReference?: string;
  status: "QUEUED" | "PROCESSING" | "COMPLETED" | "FAILED" | "RECONCILED";
  attempts: number;
  maxAttempts: number;
